...
- **Task 3** — `MarginScreen`: transient `journal` field; safe-pause routing in `handleInput` (after the menu-CLOSE block but before the menu-open check — corrected by review; menu and Journal truly mutually exclusive, menu-open swallows J, journal-active swallows M and the ESC-for-menu); J toggles open, J/ESC closes while active, R restarts (dialogue precedent), all other keys swallowed (AD-14); `renderJournalPage()` renders `journal.entries(state)` (title + status + wrapped objective + "[J] close" footer; "No threads yet." when empty) between the dialogue branch and the event-window fallback; `restart()` closes the Journal.
- **Task 4** — `JournalControllerTest` (10 + 1 review pin): the discovery trigger (capture → note → first read starts the quest, LINE_STARTED announced once, no turn, note stays, entries lists The Road East ACTIVE), the capture-flag precondition (no capture → lore reveals but no quest), the reveal-only second read (no re-announce), the passive derivation with precedence (started/completed/voided on a synthetic quest — the Journal holds no state), the NPC-line trigger through the REAL `DialogController` (`withFlag` → SetFlag → flag set → ACTIVE), void-on-giver-death (`npc.<giver>.dead` → VOIDED) + the scripted-void-wins pin, unstarted-unlisted, an unstarted quest with a terminal flag is not listed (review pin), safe pause (open/close ticks nothing — clock + hunger/thirst/temperature/hp unchanged), and the AD-6 round-trip (a reloaded run starts with no Journal open; the started flag survives and re-derives ACTIVE).
- **Task 5** — Full suite green: 299 tests after review patches (was 288), 0 failures. No new persisted `RunState` field (AD-6 by construction — quest state rides the existing FlagStore; the controller/surface are transient). Persistence and AD-5 placement suites stay green — the note branch adds no RNG draw. Launch boot-check clean.
...
- **New:** `core/src/test/java/com/margins/rogue/narrative/JournalControllerTest.java` (11 tests — 10 dev + 1 review pin)
- **Modified:** `core/src/main/java/com/margins/rogue/system/TurnEngine.java` (the note-read quest-start branch in case USE)
- **Modified:** `core/src/main/java/com/margins/MarginScreen.java` (journal field + import + safe-pause routing + J key + `renderJournalPage` + `restart()` close; review — input-routing reorder, J affordance, statusLabel switch expression, panel-bounded entry loop)

### Review Findings

Code review run 2026-08-09 (3-layer: Blind Hunter + Edge Case Hunter + Acceptance Auditor). One HIGH defect surfaced by review that was invisible to the headless suite because it lives in the SCREEN layer: the Journal input block sat AFTER the menu-open check, so with the Journal open ESC/M opened the menu instead of closing the Journal — the documented ESC close was dead and the surfaces were not mutually exclusive (the code comment even claimed the opposite). The Edge Case Hunter's mutual-exclusion verification was wrong (it assumed an ordering the code didn't have); the Blind Hunter caught it. A MED contract gap (unstarted quests with terminal flags listed as VOIDED/COMPLETED) and five LOW hardenings were also fixed. One MED test gap deferred (screen-level input harness). The Acceptance Auditor verified all ACs and Design Decisions cleanly. All applied findings checked; suite 299 green; boot clean.

- [x] [Review][Patch] **HIGH — ESC/M with the Journal open opens the Menu instead of closing the Journal** [`core/src/main/java/com/margins/MarginScreen.java:352`] — the journal-active block and J-open check sat AFTER the `if (down(M) || down(ESCAPE)) menuOpen = true` check, so with the Journal open, ESC/M hit the menu-open branch first: the menu opened over the still-open Journal and the documented "J or ESC → close" was dead for ESC. Fixed: the Journal block + J-open check now sit after the menu-CLOSE block but BEFORE the menu-open check — ESC with the Journal open closes it, M is swallowed, and the menu and Journal are truly mutually exclusive (menu-open swallows J; journal-open swallows M and ESC-for-menu).
- [x] [Review][Patch] **MED — `entries()` lists a quest the player never started (as VOIDED/COMPLETED)** [`core/src/main/java/com/margins/rogue/narrative/JournalController.java:103`] — the VOIDED/COMPLETED branches ran before the started guard, so a terminal flag (scripted void, dead giver, completion) on a never-started quest produced a ghost Journal entry, contradicting Decision 8 ("a registered-but-unstarted quest does not appear"). Fixed: listing is gated on `startedKey != 0` first, then precedence applies among started quests; javadoc updated. Pinned by the new `anUnstartedQuestWithATerminalFlagIsNotListed` test.
- [x] [Review][Patch] **LOW — `renderJournalPage` bounds only the objective wrap, not the entry title/status** [`core/src/main/java/com/margins/MarginScreen.java:1495`] — a multi-entry list (a later Epic registers more quests) could draw titles below the panel and collide with the "[J] CLOSE" footer. Fixed: the whole entry body (title + status + objective) is panel-bounded — the break now precedes the title draw.
- [x] [Review][Patch] **LOW — no in-game affordance reveals J opens the Journal** [`core/src/main/java/com/margins/MarginScreen.java:1389`] — the only "[J] CLOSE" hint appears after the Journal is already open. Fixed: added "J  Journal" to the menu CONTROLS panel (right column, mirroring the left column's 4th row).
- [x] [Review][Patch] **LOW — `JournalController.register` accepts blank/duplicate quest ids** [`core/src/main/java/com/margins/rogue/narrative/JournalController.java:70`] — a duplicate id would render two Journal rows deriving from the same flags; a blank id builds `quest..started`. Fixed: blank ids and duplicates rejected (latent today — the ctor seeds once).
- [x] [Review][Patch] **LOW — `statusLabel`'s `default` silently maps any unknown status to "VOIDED"** [`core/src/main/java/com/margins/MarginScreen.java:1514`] — a future 4th `QuestStatus` would render as VOIDED with no error. Fixed: converted to a switch EXPRESSION — compile-time exhaustive over the enum.
- [x] [Review][Patch] **LOW — R (restart) is swallowed while the Journal is open, unlike every other text surface** [`core/src/main/java/com/margins/MarginScreen.java:352`] — the game-over and dialogue surfaces both restart on R; the Journal swallowed it. Fixed: R-restart added to the journal-active block (restart() already closes the Journal).
- [x] [Review][Defer] **MED — the AD-14 safe-pause test is vacuous — no screen-level input-routing test** [`core/src/test/java/com/margins/rogue/narrative/JournalControllerTest.java:183`] — the test pins the controller contract (open/close mutates nothing) but not the screen's enforcing swallow branch, so it could not catch the HIGH routing bug above. A screen-level Gdx input-routing test needs a headless Gdx backend/Mockito harness the repo doesn't have (the existing screen-layer test only exercises static pure functions) — new test infra, deferred. The routing defect itself is fixed and boot-verified.
...
| 2026-08-09 | Dev | Implemented all 5 tasks. New `JournalController` (passive lookup, no quest-state copy) + the discovery trigger in `TurnEngine` (first Torn-Page read after the capture starts "The Road East", announced, no turn, note stays) + the `MarginScreen` Journal surface (transient field, safe-pause routing, J open / J+ESC close, `renderJournalPage`, restart close). 10 new tests pin AC-1 (discovery, NPC-line through the real DialogController, void-on-giver-death + scripted void), AC-2 (passive derivation with precedence, unstarted-unlisted), AD-14 (open/close ticks nothing) and AD-6 (round-trip keeps the started flag; reloaded run starts with no Journal open). Full suite green: 298 tests (was 288). No new persisted `RunState` field (AD-6 by construction). Status → review. |
| 2026-08-09 | Review | 3-layer code review (Blind Hunter + Edge Case Hunter + Acceptance Auditor). HIGH: the Journal input block sat after the menu-open check, so ESC/M with the Journal open opened the menu instead of closing it — fixed by reordering the block after menu-CLOSE but before menu-OPEN (surfaces now truly mutually exclusive; ESC closes the Journal). MED: `entries()` listed unstarted quests with terminal flags — gated on `started`, precedence now applies among started quests, pinned by a new test. Five LOWs fixed: panel-bounded Journal entry loop, "J Journal" menu affordance, `register` blank/duplicate-id rejection, `statusLabel` switch-expression exhaustiveness, R-restart in the journal block. One MED deferred (screen-level Gdx input harness — no headless backend in the repo; the routing defect itself is fixed + boot-verified). Suite 299 green (was 298; +1 review pin); boot clean. Status → done. |
