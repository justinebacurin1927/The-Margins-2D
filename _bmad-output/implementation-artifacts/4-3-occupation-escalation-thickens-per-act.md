...
- [ ] **Task 1 — Act flag + accessor in FlagStore (AC-3, D1).**
  - [ ] 1.1 Add `public static final String KEY_ACT = "act.current";` and a typed `public int getAct()` returning `Math.max(1, get(KEY_ACT))` (0/never-set → Act 1, the AD-6 default). Optionally a `setAct(int)` clamped to ≥1 for the test path / Epic 5. Mirror the `KEY_BOND`/`getBond` pattern; document that Epic 5's quests own the writes (AD-11).
  - [ ] 1.2 Confirm no new `RunState` field and no `restoreAfterLoad` change — the flag rides the existing serialized `flagStore` map, so a pre-4.3 save reads Act 1 by construction.
- [ ] **Task 2 — Per-act, cordon-aware density function (AC-1, AC-2, D2/D3).**
  - [ ] 2.1 Widen `enemyCountFor` to `enemyCountFor(float eastness, int act, float ny)`: compute the existing base step, then `if (act > 1 && base > 0 && !inCordon(eastness, ny)) base += (act - 1);` return base. Keep it a **pure static** — no rng, no state.
  - [ ] 2.2 Add a small pure `inCordon(float eastness, float ny)` → `eastness < 0.2f && ny > 0.8f` (the `WorldSpine` BORDER_X/BORDER_Y corner). Comment that it is explicit-but-redundant with the west safe tier (documents the AC-2 channel split at the site).
- [ ] **Task 3 — Wire the ramp into placement (AC-1).**
  - [ ] 3.1 In `placeFloorActors`, read `int act = getFlagStore().getAct();` once before the loop; per region compute `float ny = cy / (result.spine.getHeight() - 1f);` (or a spine helper) and call `enemyCountFor(spine.eastness(cx), act, ny)`. Leave the per-enemy position-draw block untouched (AD-5 call structure unchanged).
- [ ] **Task 4 — Tests + verification (all ACs).**
  - [ ] 4.1 AC-3: `FlagStore.getAct()` returns 1 when unset and after a round-trip of an unset store; returns 2/3 after `set(KEY_ACT, …)`. Act-1 parity: `enemyCountFor(e, 1, ny)` equals the pre-4.3 bands for representative eastness values (no regression).
  - [ ] 4.2 AC-1: for an interior eastness (base > 0), `enemyCountFor(e, 2, ny)` = base+1 and `(…, 3, …)` = base+2 (additive, monotonic).
  - [ ] 4.3 AC-2: west safe tier (`eastness ≤ 0.2`) returns 0 at acts 1/2/3; the cordon box (`eastness 0.05, ny 0.9`) returns 0 at all acts; a would-be interior point forced into the cordon box is not bumped.
  - [ ] 4.4 AC-1 end-to-end (regen proof): build a `RunState(seed)`, set act 3, `generateFloor()`, and assert the eastern rooms hold strictly more enemies than an act-1 regen of the *same seed* — the mechanism bites when a trigger (here, an explicit regen) fires. AD-5: same seed + same act regenerated twice → identical enemy layout.
  - [ ] 4.5 Full suite green via `docs/BUILD.md` (`mvn -o clean install`), no regressions (currently 427 tests). **Verify:** all green, boot clean.
