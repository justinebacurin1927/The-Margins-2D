...
## Deferred from: code review of story-6.4 (2026-08-17)

- **[Med] Trader roaming/mobility is deferred — traders are stationary per-seed points.** The two "mobile" traders are placed as fixed tile-agents (no map field, no AI); the "mobile" flavor is only that they are open-map points rather than shop buildings. A real roaming/wander AI (and its determinism + save re-injection) is future work. [RunState.placeTraders, Trader]
- **[Med] Bespoke shop UI is deferred.** Interaction is a minimal safe-pause `MarginScreen` panel (`[T]` open when adjacent, buy-by-number, `[K]` sell, `[B]` barter, `[T]`/ESC close) + tinted trader glyphs for findability. Animated trader sprites, a multi-mode shop screen, a large stock catalog, restock/economy simulation, and VOICE-gated haggling are deferred — the 6.1/6.2 UI-deferral precedent. The AC-critical logic is the fully-tested headless `TradeSystem`/`Trader`. [MarginScreen.renderTradePanel/handleTradeInput]
