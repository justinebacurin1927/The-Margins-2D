...
- **The field-absent migration rule (standing dev-notes rule — codified after it bit twice).** libGDX `fromJson` runs the no-arg `RunState()` ctor, which delegates to the seeded ctor and *draws gameplay state* — so a save **missing a new persisted field** does not load a "default"; it inherits **nondeterministic ctor-rolled state** (nanoTime-seeded). Therefore: **every new persisted field ships with EITHER a deterministic default (a field initializer / a load-time clamp) OR a load-time reconcile in `restoreAfterLoad()` that derives it from persisted state.** The wart bit on `weather` (Story 1.3: a field-absent save rolled a nanoTime weather, only the cycle was derivable and reconciled) and is the same pattern as the deferred 3.3 `identifyMap` binding. Check this rule in any story's dev notes before it adds a `RunState` field.
